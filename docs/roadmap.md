...
- [ ] **会话块化（立项一，实机靶心）**：真实使用反馈——「结论和思考/执行命令搅在一条流里，结论被埋」。改为：你的输入=块首，**结论聚合亮顶**，思考/执行细节折叠成一列徽标行（⚙ N · ⏵ Ns · ⛁ N）按需展开。块化连带 thinking 展开（当前只有一行不可展开，1.1.9 遗留）。
- [ ] **决策面板泛化**：boundary ask 之外加「分歧提请」（模型先查专家知识，有基准不问人）；决策卡带专家依据区（命中亮出 title/criteria + E#N，未命中标注「库中无基准」）；**输入为主通道**（a/b 快捷键为辅——人常答「都不是，选 C」）；拍过的板落成会话里的决策块（可追溯 + 日后 promote 入口）。
- [ ] **验收**：全量测试 + pty 实机回放 harness（winpty 驱动真 TUI 跑固定场景，改动前后可对比）+ 用户真实任务 dogfood。

> 明确不做：信息面板/常驻状态板（用户拍板砍掉）、侧栏（后续候选）、主题皮肤（未拍板）、chrome 布局大改（输入框保持底部——连续驱动面）、/bg、switchHook。
> 后续候选：侧栏会话线导航、块目录跳转、@ 补全扩展、问历史、视口漂移根治、Ctrl+O/Ctrl+Z 等键位小项。
