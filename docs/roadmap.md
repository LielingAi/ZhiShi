...
> 技术面待定稿：窗口/IPC/sidecar 联动（Rust 侧）、SSE 直连 vs IPC 转发、块数据语义（输入=块首/结论聚合/细节清单）、**多环境分层（用户确认分层走）：A 多会话视图（1.3.3 随 GUI 迭代） vs B 引擎真并行 turn（单独立版本，原 2.0 前置项，不与 GUI 迭代搅版）**。
