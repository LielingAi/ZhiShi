...
- [ ] **/ 命令操作面**：inline 参数直输（如 `/snapshot myname`、`/intel CVE-2024-1234`——overlay 选中后把光标后剩余文本作为参数直发，免开模态；无参数时才回落模态收集）；命令面板分组排序（研究组 /intel /archive /decide 靠前）。
- [ ] **@ 命令操作面**：与 / 面板交互对齐（前缀过滤 + 分组标签统一）；@ 文件路径补全防抖/缓存体验核查（1.3.3 既有机制，实机确认）。
