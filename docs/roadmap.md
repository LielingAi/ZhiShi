...
## 1.6.2 —— crash-triager 深挖模式（崩溃变体分析）
...
- [ ] **crash-triager.md 深挖模式**：五步流程（复现基准指纹 → 根因结构化（过门）→ 同族扫描（根因模式→检索模式）→ 定向构造+验证门（崩溃且栈指纹≠种子，硬标准）→ 分流（新类走批处理入口独立落 research_events；假设耗尽 outcome=fail 落负结果））+ 产出纪律（变体落 `triage/variants/`）
- [ ] **stack-hash.sh**：栈指纹工具（信号+顶 N 帧 hash），fuzz / fuzz-vm 配方各一份
- [ ] **fuzz-runner.md 一句话**：深挖产出的验证变体落 `fuzz/corpus-in/` 时下一轮带上（闭环最后一环）
- [ ] **dogfood：第二 bug 家族复测**：glibc iconv（CVE-2024-2961）SS2 种子 → 独立产出 SS3 家族并归类（判据同 1.6.1：崩溃且栈指纹≠种子 + 人工复核命中）。双重目的：①复测机制（样本量债务，1.6.1 §6.5-5）；②裁决 A/B/C 接线方案（A 跑通则零接线盖棺，退化则失败现场作 B/C 立项依据）。**记忆污染声明**：CVE-2024-2961 公开披露、模型可能背过——结论权重低于注入靶，记录在案
