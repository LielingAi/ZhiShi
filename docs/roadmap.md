...
- [x] **#1 turn 空产出检测与续跑**：turn 结束时判定「零可见内容 + 零工具调用」（thinking 烧穿形态）→ 自动续跑一轮（注入「刚才的输出被截断/空了，继续」类提示）+ 达上限（防死循环）则升级人（request_decision/明确报错广播）+ GUI 可见（空 turn 不得静默）；thinking 截断的检测信号（finish_reason=output_limit）进判定
- [x] **#2 exec 超时杀进程**：env_exec 超时后强杀远端进程（posix `timeout`/kill 树包装、windows taskkill /T /F——bg-exec 已有同族件可参照）；通道自堵检测（连续 N 次超时）→ 明确报错 + 升级
- [x] **#3 环境故障升级纪律**：kernel/教学段写死「环境故障 N 分钟（或 M 次探测）未恢复必须升级人（request_decision），不得静默自扛」——纪律件 + 轨迹实证案例入注释
