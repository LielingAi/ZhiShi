...
- [ ] **GUI MVP**：Tauri 开窗（现壳无窗口）+ webview 会话视图——块化流（你的输入=块首、结论聚合亮顶、思考/执行细节折叠成徽标行、Ctrl+O 类交互展开）、输入与发送、状态行。SSE 客户端照 `src/cli/tui/client.ts` 契约重写（纯搬运）。**第一版不引前端框架**（纯 HTML+TS，打包静态文件进 resources，与 CLI 打包同径）——最小验证跑通链路后视迭代需要再评估框架。
- [ ] **最小验证先行**：开窗 + 静态页接 SSE 会话流（几百行）——链路可行即出 MVP 全量。
- [ ] **TUI 冻结**：不投功能/视觉，只修致命 bug。
- [ ] **验收**：全量测试 + 手动走查（开窗/会话流/输入/状态）+ 打包安装实机验证。

> 明确不做（本版）：TUI 块化与视觉投入（已止损）、决策面板（后续迭代，GUI 原生做）、信息面板、侧栏、主题、TUI 退役（GUI 覆盖核心场景后）。
> 后续迭代：决策面板（越界+分歧提请+专家依据）、历史面板、@ 补全、问历史、侧栏、主题；GUI 覆盖会话+决策+历史后退役 TUI。
