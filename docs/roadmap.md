...
## 1.2.4 —— 蒸馏弧效果验证与修正（进行中）

方向切换（用户拍板 2026-08-22）：1.2.4 起**做深不做广**，不迭代新功能，做到能用、能打。首选最深的欠账：蒸馏弧是 harness 三件内建之一（§7「让循环越转越好」），但 1.1.3 立了度量口径就「量化暂缓等数据」——「经验反喂有没有用、有没有反作用」至今无证据。

- [ ] **链路审查**：research_events → 蒸馏弧（distill.ts / distill-research.ts）→ 蒸馏经验 → 逐 turn 反喂系统提示——每个环节实际在做什么、反喂进 prompt 的实际内容质量如何（先读后评，不许凭印象）。
- [ ] **对照度量**：用 1.1.3 现成口径（重复踩坑率 / 回合数，可比任务组）——同类任务跑无反喂 vs 有反喂两路，用攒下的真实 research_events 当蒸馏原料。
- [ ] **按证据修正**：没用就修或砍（诚实面对），有用就加强（反喂内容质量 / 时机 / 域隔离粒度）。
- [ ] **CVE 复现 dogfood**（D21 欠账，顺手）：挑一个 CVE 完整复现一遍——既还 dogfood 欠账，又是蒸馏验证的真实任务载体。

> 明确不做：新功能、新域（红队/恶意软件重启评估继续暂缓）、发行链路（UPDATE 暂缓）、1.2.x 度量期（等真实使用数据）。
...
- [x] **引用追踪 + 冲突记录**（飞轮的仪表盘）：research_log 可关联 expert entry id（「本次决策依据 E#N」）；报告（1.2.0 出口）标注引用的专家条目；LLM 判断与专家知识冲突时记录冲突点。没有这层，1.2.3 的度量无数据可量。
