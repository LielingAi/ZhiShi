...
- [ ] **#5 R1 子代理可发现性**：系统提示加「可委派子代理」段（按域收窄的名字+一句话用途+何时用）；delegate_task 描述补场景提示（长跑 fuzz/批量崩溃分拣应委派）
- [ ] **#5 R2 bg 完成回注 loop**：chat:bg-finished 注入 loop（消灭 sleep 轮询——单会话 47 次/40% 墙钟的浪费）
- [ ] **#5 R3 环境 select 探活**：选中时一次 env_exec 探活，鉴权失败当场报 + 引导 1.6.5 密码引导修复
- [ ] **#5 R5 小件**：intel 未构建引导（会话内首查即提示 update）；research_events task_kind 加 `fuzz`（可观测性）
...
## 1.6.8 —— 战役原语（discovery campaign）（预立项，待 1.6.7 dogfood 取证）

**缘起（2026-09-09 用户拍板）**：轨迹裁决——挖掘无效的根因是系统没有「发现式搜索」的任务形态。挖掘不是更长的任务，是**战役**：假设队列 + 搜索基座（fuzz 是之一）+ 信号反馈（覆盖率/崩溃/指纹）+ 分支剪枝 + 墙钟预算（天级非轮次级）+ 事件驱动的 agent 介入（信号驱动非轮次驱动）。零件都在（auto loop/research_archive/env_bg/fuzz-runner），缺编排原语。

- [ ] **设计稿**：`docs/design/discovery-campaign.md`——输入必须是 1.6.7 dogfood 的失败清单（零负样本不画架构图，1.6.1 纪律）
- [ ] **战役原语实现**：按设计稿

> 边界：设计稿完成前不写实现代码；1.6.7 接线件（R1~R3/R5）是战役原语的地基，先行落地。
