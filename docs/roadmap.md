...
用户定调（2026-08-26）：深入环境、配方的功能实现与 GUI 调用实现，寻找 BUG，环境多配方。1.3.7 把环境业务逻辑收干净后，本版把环境与配方两个对象的功能面做深。

- [ ] **环境生命周期全操作 GUI 化**：侧栏环境行操作面——启动/停止（down）/快照（snapshot）/回滚（rollback）/提取（extract）/删除（rm，1.3.7 已做）/能力刷新/详情查看；运行中/已停止/本机已有三态判定统一（1.3.7 调研混乱点 #5 的收口）。
- [ ] **配方功能 GUI 化**：配方详情展示（工具清单/工作流摘要/域绑定/引擎依赖）、domain check 可达、配方与环境关联展示（哪个环境用了哪个配方 + 工具漂移状态）。
- [ ] **环境/配方 BUG 普查**：up/down/ps/discover/snapshot/rollback/exec/rm/adopt/build 的边界 + GUI 接线正确性，实机驱动 + 代码审查，产出 bug 台账逐条定性修复（方法论同 1.2.8 TUI 普查）。
- [ ] **环境多配方**：`recipeId` 单值 → `recipeIds[]`（绑定侧多配方关联——能力清单/工具检查/域基线按配方集合；与 1.3.7 场景 3 推导域集合的分工：绑定=意图、推导=现场）。组合构建（多配方装一个环境）另行评估，本版先做关联侧。
