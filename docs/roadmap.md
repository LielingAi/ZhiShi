...
> 版本任务池。1.0.0 = 安全研究员版定型；1.1.x = 能力补齐 + 引擎深化（已完成）；1.2.x = 研究交付 + 校准协作主线 + 做深不做广（已完成）；1.3.x = GUI 主线（会话/决策/历史 + 环境业务逻辑 + TUI 退役 + 质量审计，已完成，1.4.0 发版）；**当前线：1.4.x（GUI 正式版后——auto loop agent 已落地（1.4.1，设计 `docs/design/auto-loop-design.md`）；1.4.3 域模型归位已发版；1.4.4 研究档案已发版；1.4.5 质量审计已发版；1.4.6 Harness 复杂任务优化已发版；1.4.7 打磨收口已发版；1.4.8 研究档案本体迭代（反证结构 + 链式投影 + 假设第三终态 + confirmed 死状态修复）已发版；1.4.9 环境元数据可信已发版（+发版后 CRLF 实机补丁 ebce87e）；1.4.10 第三方 API 支持已发版（issue #6 关闭）；1.5.0 触发权归人已发版（/expert /intel /archive /decide + auto loop 检查点 + kernel 瘦身 2700→2450）——1.4.x「模型主动」路线实证失败（决策 0 调用/专家 1 会话/档案全靠人触发），用户拍板转向。记录在案：god file 拆分、引擎单例残余耦合（纯技术债，无 2.0 绑定——「引擎多实例」已随 2.0 范围砍单，2026-08-27 决策）。已砍：/bg 转后台（2026-08-26 决策——auto loop + env_bg 全覆盖其价值）、决策面板澄清提问（2026-08-27 决策——澄清是输入不是决策，普通对话打字即交互、auto loop 自主含义不允许向人要信息）、引擎多实例（2026-08-27 决策——与 TUI 多线同屏、跨机协同调度一起，2.0 范围砍单））**。
> 状态约定：`[ ]` 未开始 · `[~]` 进行中 · `[x]` 已完成。任务细节不写在这里——细节进对应设计文档。

---

## 1.6.9 —— 引擎健壮性（假死/通道自堵/环境故障升级纪律）（进行中）
...
- [ ] **#1 turn 空产出检测与续跑**：turn 结束时判定「零可见内容 + 零工具调用」（thinking 烧穿形态）→ 自动续跑一轮（注入「刚才的输出被截断/空了，继续」类提示）+ 达上限（防死循环）则升级人（request_decision/明确报错广播）+ GUI 可见（空 turn 不得静默）；thinking 截断的检测信号（finish_reason=output_limit）进判定
- [ ] **#2 exec 超时杀进程**：env_exec 超时后强杀远端进程（posix `timeout`/kill 树包装、windows taskkill /T /F——bg-exec 已有同族件可参照）；通道自堵检测（连续 N 次超时）→ 明确报错 + 升级
- [ ] **#3 环境故障升级纪律**：kernel/教学段写死「环境故障 N 分钟（或 M 次探测）未恢复必须升级人（request_decision），不得静默自扛」——纪律件 + 轨迹实证案例入注释

> 边界：不做 exec 通道并发化（架构重写，1.6.9 只做超时杀+自堵检测）；记忆层读侧（archive 读 op/recall 接线）与武器化慢循环对策是 1.6.8 之后的专项，不在此版。
> 验收：#1 有「截断 turn → 自动续跑/升级」的测试证据 + mtst9nns 形态回归（16s 纯 thinking 不再静默）；#2 超时强杀有测试；#3 纪律进 kernel 段。

---

## 1.6.8 —— 战役原语（discovery campaign）（代码完成已发版，待实机验收实验）
...
## 1.6.7 —— 实机反馈修复 + 挖掘链路接线 + fuzz dogfood 实验（完成——判定：接线全部落地，dogfood 成立，报告 docs/design/experiment-fuzz-dogfood.md）

**缘起（2026-09-09 用户实机反馈四条 + 两轮轨迹分析）**：1.6.4~1.6.6 发版后实机 dogfood 暴露三条显示/语义 bug（#1~#3）；第四条「研究有效、挖掘无效」经两轮轨迹分析（25 事件 + 17 会话 + 6 天日志）裁决：**挖掘链路从未真正启动**——设计层缺「发现式搜索」正向回路（剧本只为复现优化）；架构层三处断线（子代理可发现性 / bg 完成不回注 / 环境单点）；实现层最后一公里（名册不进 prompt、intel 未建索引）。agent 能力被轨迹反证（x1→x50 受控迭代打出 root）——不是模型问题。用户拍板：1.6.7 只做接线 + 取证，结构件（战役原语）挂 1.6.8 预立项——按 1.6.1 实证纪律，零负样本不重设计。

- [x] **#1 运行中环境别名显示**：label 优先级反转（别名压住引擎实例名，docker/hyperv/vbox 同修）+ AttachView 头显别名；envTag 安全锚走机器名 by design 不动（a1bcf0b）
- [x] **#2 /reset 语义核查与对齐**：查实——清上下文 + 新轨迹懒惰落盘 + 旧线封存（语义自洽）；真坑是历史面板死行（回看 404/载回开空线）→ 修复：解绑时旧线 id 存 `archivedLoopSessionId`，只读回看回落，恢复/载回不消费（防复活纪律不变）；user-guide 补全破坏性副作用与 env reset 撞名区分（a1bcf0b）
- [x] **#3 版本展示面归一**：README 头部版本行进 sync-version.js 同步链（硬失败纪律同款；1.4.8/1.4.9/1.6.6 第三次犯，四面全收）（a1bcf0b）
- [x] **#4 漏洞挖掘有效性分析**：两轮轨迹分析出书面结论（见缘起裁决）——五断点 + 三层裁决，证据带轨迹引用
- [x] **#5 R1 子代理可发现性（74d208a）**：系统提示加「可委派子代理」段（按域收窄的名字+一句话用途+何时用）；delegate_task 描述补场景提示（长跑 fuzz/批量崩溃分拣应委派）
- [x] **#5 R2 bg 完成回注 loop（74d208a）**：chat:bg-finished 注入 loop（消灭 sleep 轮询——单会话 47 次/40% 墙钟的浪费）
- [x] **#5 R3 环境 select 探活（74d208a）**：选中时一次 env_exec 探活，鉴权失败当场报 + 引导 1.6.5 密码引导修复
- [x] **#5 R5 小件（74d208a）**：intel 未构建引导（会话内首查即提示 update）；research_events task_kind 加 `fuzz`（可观测性）
- [x] **#6 fuzz dogfood 实验**：真目标端到端（harness → AFL 后台长跑 → 崩溃去重 → crash-triager 分拣）——不是验收门是**实验**：它怎么死、死在哪，就是 1.6.8 战役原语的一手需求。**判定成立**（报告 `docs/design/experiment-fuzz-dogfood.md`）：配方两坑实证修复（dea51d1）；核心数据点——盲跑 121k execs 0 崩溃 vs 引导触发秒级命中（与 1.6.1 对照实验互证：挖掘的第一变量是引导不是算力）；1.6.8 首需求 = 「零崩溃时的破冰回路」（盲跑基座 + 平台期触发引导）

> 边界：不动战役原语设计（1.6.8 取证后动工）；环境聚合（多对多合成）暂缓——dogfood 若暴露环境摩擦为瓶颈再升优先。
> 验收：R1/R2/R3/R5 有测试证据；#6 出实验报告（成功/失败明账，失败点即 1.6.8 输入）。

---

## 1.6.6 —— 环境改名（别名）（完成）

- [x] **environment/rename + CLI/GUI 双入口**：registry `renameEnvironmentEntry` 纯函数（只动 name，id 身份不改；空名清除别名）；admin 端点落盘；CLI `env rename`；GUI 侧栏 ⋯ 菜单「✏ 重命名…」模态。新增 9 测试，全量 2523 绿（41ae505）

---

## 1.6.5 —— 凭据门槛消除（密码引导 → 密钥落地）（代码完成已发版，待用户实机验收）
...
- [~] **M0 通道层补全**（代码完成，待用户实机验收）：能力探测 PowerShell 协议（`recipes.ts` TOOL_PROBE_COMMANDS/buildToolCheckScript 按 osFamily 分派 + `capability-derive.ts` 接线）；`resolveEnvOpenCommand` 消费 osFamily；extract basename 兼容 Windows 路径；vmrun `copyFileToGuest` 封装 + `environment/push` 端点；vm-guest-exec windows 分支补单测（现零覆盖）。**验收门**：手备 Win VM（OpenSSH Server + VMware Tools）登记后 up/exec/open/push/extract/探测全通
- [~] **M1 配方 ps1 化 + `pwn-win` 配方**（代码完成，待用户实机验收）：frontmatter 认 `setup.ps1`；`provision.ts` 补齐链路 PowerShell 分支（`-EncodedCommand` 替代 base64|bash，sudo 预检换管理员组判定）；`bundled-environments/pwn-win/`（SKILL.md + setup.ps1，幂等可重放；包管理器 choco 兜底 + winget 优先——裸 VM 常缺 App Installer；必装段：Git/Python/LLVM(clang-cl)/VS Build Tools(VCTools)/cdb(SDK Debugging Tools)/Sysinternals/Ghidra headless/procdump；可选段：WinAFL+DynamoRIO、x64dbg；`_NT_SYMBOL_PATH`/WER LocalDumps/gflags PageHeap 封装；Defender 排除项引导）。**验收门**：实机重放 setup.ps1 幂等，探测全 OK
- [~] **M2 Windows adopt**（代码完成，待用户实机验收）：`vm-adopt.ts` Windows 路径——Add-WindowsCapability 启用 OpenSSH Server → 建 `researcher` 用户 → 公钥落位（`administrators_authorized_keys` 怪癖：管理员用户 key 落 ProgramData 不落用户目录，ICACLS 权限位）→ 跑 setup.ps1 → `shutdown /s` → `zhishi-clean` 快照。**验收门**：干净 Win VM 一条 `env adopt` 养成；之后 `env up` 回滚干净现场——与 Linux 体验同构
...
- [x] **crash-triager.md 深挖模式**：五步流程（复现基准指纹 → 根因结构化（过门）→ 同族扫描（根因模式→检索模式，含扫描输出收敛纪律——dogfood 首轮暴露后补，161b050）→ 定向构造+验证门（崩溃且栈指纹≠种子，硬标准）→ 分流（新类走批处理入口独立落 research_events；假设耗尽 outcome=fail 落负结果））+ 产出纪律（变体落 `triage/variants/`）
- [x] **stack-hash.sh**：栈指纹工具（信号+顶 N 帧 hash），fuzz 配方 `examples/` + fuzz-vm `setup.sh` 内嵌同文本，SKILL 双双提及（02ca497）
- [x] **fuzz-runner.md 一句话**：深挖产出的验证变体落 `fuzz/corpus-in/` 时下一轮带上（02ca497）
- [x] **dogfood：第二 bug 家族复测**：glibc iconv（CVE-2024-2961）SS2 种子（`2A48`）→ 独立产出 SS3 变体（`2B49`/`2B4A`，plane-3 字符甚至不在人工预验清单）。两轮：首轮 150k 累计预算耗尽在扫描阶段（暴露扫描收敛缺口 + 预算口径记录 + 验证文件误留工作区卫生事故），第二轮修复后通过（发现点 ~242k 累计 / 84s）。判定与污染审计见 `docs/design/experiment-crash-variant.md` §7。**A/B/C 接线裁决：A 零接线盖棺，B/C 维持不立项**

> 边界（不做）：语料扰动变体生成（实验 1 变体即中，dogfood 证明需要再加）；whitebox/expert 任何引用；批处理模式一行不改；引擎/GUI/auto loop 不动；预算数字不进 agent 定义。
> 验收：iconv 复测按锁定判据过/不过明账；不过 = 机制不成立，回炉不标 done。
...
> 验收：golang 会话尾段重放压测全过（收割/指针/recall/零雪崩/用户指令零裁剪）+ 全量测试。
> 风险与缓解：收割规则漏提 → 指针卡带行区间可 recall 原文兜底；recall 滥用 → 教学 + 预算截断；压缩中枢回归面 → 六场景 + 新场景双层钉死。

---

## 1.5.2 —— GUI 体验优化：设置页自适应 + 会话滚动操作 + 斜杠/@ 操作面（已完成）
...
- [x] **harness 确定性注入（唯一注入路径；图论口径）**：**注入 = 以当前焦点节点为锚的邻域子图投影**——节点 = 档案实体/专家条目/消息/工具，边 = 引用/驱动/证据/域先验/关键词相似（弱边即时计算，不建持久图）；每轮取焦点（pending H# / open Q# / 最近用户消息）的 k=1~2 邻域，边类型加权（证据边 > 派生边 > 域先验边 > 关键词相似边）+ 会话内去重（同条目只注一次）→ 注入 1-2 条（criteria 截断 + 总字符硬顶）；**零注入语义**（邻域为空静默）；**透明标注**（流内可见，注错人可纠正）；确定性（同输入同输出）；auto loop 同规则。
- [x] **skills 层删除**：bundled-skills 方法论**策展提炼**为专家条目（每域 5-15 条，判据化补齐 criteria/applicability——提炼不是搬运；**迁移映射表（每条 bundled skill → 专家库/工具侧）为开工前置评审项**）；工具用法内容随工具/配方分布；删除 buildSkillsSection / SKILLS_TOTAL_CAP / filterSkillsByDomain / 启禁管理 / GUI skills 页签（并入专家页签）/ CLI skill 命令；domain.json `skills[]` → 检索域先验；用户库 skills 迁移为用户专家条目（provenance=user）。
- [x] **MCP 删除**：GUI MCP 页签、CLI `zhishi mcp`、服务端 mcp 路由、mcp-oauth、config mcp 段、plugin-bridge 资源。开工前置：确认 config.json 无在用 MCP server。
- [x] **/expert 命令删除**：slash 注册/路由/执行/单测移除（1.5.0→1.5.1 迭代轨迹写明）；/intel /archive /decide 保留。
- [x] **专家库判据化**：入库硬校验（provenance/reviewer/criteria/applicability 四必填，缺一拒绝；草稿区与审定区分层）；检索 top-N + 域先验排序防稀释。
- [x] **回归 + 验证**：注入规则单测（阈值/去重/零注入/预算/确定性/域先验）；删除面无残留引用（typecheck + eslint + depcruise）；迁移条数目视评审；全量测试 + 构建；实机（真实会话观察注入块时机/误命中率/auto loop 检查点联动）。

> 实际落地（2026-08-30）：①**harness 确定性注入**——新模块 `loop/expert-inject.ts`（图论口径邻域投影：焦点锚点 = pending H#/open Q#（≤2+≤2）+ 最近用户消息；确定性分词（拉丁 token + CJK bigram + 停用词表）；边权重 档案 3 > 域 2 ≈ 用户 2，阈值 4；top 2 + 总字符硬顶 1200；内存态会话去重；零注入语义；透明标注列 #id + 「不适用/注错了请无视并继续」）；接入 `assemblePiSystemPrompt`（security/auto-run 同规则）；kernel 口径同步「harness 按焦点自动注入 + 判据验证」（2447 字符，未抬顶）。②**skills 层删除**——注入段/预算/域过滤/启禁管理/GUI SkillsTab/CLI skill 组/skill 管理 API（~345 行）全删；扫描器与 .claude 兼容同步保留（边界）；迁移映射按评审表执行：5 域方法论策展提炼为 **6 条判据化专家条目**（whitebox 决策链+不报告清单、binary 利用链+崩溃研判、pentest 渗透链、ai-security 评估链——gdb -batch/-Pn/cyclic/拒绝≠安全等实战细节保留），native-code-loop（与 kernel 重复）删除、task-*（工作法与 kernel 重叠）删除、agent-browser/zhishi-cli/download-anything/range-ops 归工具侧（本体保留）。③**MCP 级联删除**——routes/mcp、mcp-oauth（7 文件）、mcp-bridge、GUI McpTab、CLI mcp 组、config-types mcp 字段族、cron/会话快照/工作区解析全链、plugin-bridge 资源；外部 runtime（Codex/CC）自身的 MCP 概念不属被删面（保留）。④**/expert 命令删除**（slash 注册/路由/执行/单测；/intel /archive /decide 保留）；expert/search 的 format:text 分支同撤。⑤**判据化硬校验**——expert/add reviewer 必填（四必填齐：provenance/reviewer/criteria/applicability，promote 预填流由人审定落入同一闸）。⑥**实机走查**——seed 11 条目 0 错误（策展 6 条 kind 修正 sop）；注入活体：栈溢出焦点命中 #5#3、AI 安全焦点命中 #1#2、去重链（同条目不重注、新条目照注、穷尽后零注入）、无关话题零注入全正确；zhishi-cli/SKILL.md 同步删 MCP/Skills 段。回归：新增 13 单测（expert-inject）+ 子代理侧 ~30 用例按新现实改写/删除；全量 182 文件 2390 测试绿 + typecheck + eslint + depcruise 480 模块零违规 + 三构建绿。（子代理交付的顺手修复：kernel 模板内联反引号语法错。）
> **分发善后（2026-08-30 用户追问「初始化专家数据/分发给别人用」牵出）**：①`seedToolSkills` 恢复工具侧 4 技能（agent-browser/download-anything/zhishi-cli/range-ops）的本体分发——子代理把 seedBundledSkills 整个删掉后，新装用户的 ~/.zhishi/skills 不再播种，工具侧技能（slash 发现 + .claude 兼容同步都吃这个目录）对新用户静默丢失；恢复为只发工具侧 4 个的窄播种（hash 幂等，syncToolSkills 核心可注入测试）。②退役 8 目录从 bundled-skills 删除（方法论已沉 bundled-expert/已删）——包内不再有双源。③domain.json `skills[]` 清空 + 语义注释（检索域先验=条目自身 domain 字段，本字段占位兼容）；domain check 语义同步。④Rust commands.rs SYSTEM_SKILLS 收窄为同 4 个 + 版本门 37→38（老安装升级强制同步到位）。⑤CR 污染清理：工具侧技能文件 ~30 个 LF 归一（agent-browser/SKILL.md 曾 \r\r\n 双回车——守卫测试首次覆盖到它就现形；1.4.9 CRLF 教训的同族，脚本带 CR 进环境必炸）。新增 3 seed 单测 + 守卫测试按新现实重写（工具侧齐备/退役不在/Rust 清单一致/版本门 ≥38）；全量 182 文件 2394 测试绿 + typecheck + eslint + 三构建绿。**用户数据口径**：已有 expert.db（自定义/晋升/草稿）零影响（seed 纪律 builtin 才覆盖、幂等键按文件身份）；新 6 条以新 id 追加；~/.zhishi/skills 的退役残留不替用户扔（文档指引手动删）。**追问「skills[] 还有什么用」牵出**：domain.json `skills[]` 字段整体删除（空数组占位是偷懒——字段已无任何消费面：json/DomainManifest 类型/loader/validate 循环/DomainCheckContext.skillIds/domain list 响应全移除；`loop/skills.ts` 模块随最后消费点消失一并删除——skills 层至此零残留）。
...
## 1.4.10 —— 第三方 API（中转站）支持 + provision 铺开（已完成）
...
## 1.4.9 —— 环境元数据可信：MISS 显式化 + 绑定域认多配方 + 已有环境补齐链路（已完成）
