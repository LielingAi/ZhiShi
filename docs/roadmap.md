...
## 1.6.7 —— 改名/版本显示修复 + 漏洞挖掘有效性分析（进行中）

**缘起（2026-09-09 用户实机反馈四条 + 分析要求）**：1.6.4~1.6.6 发版后实机 dogfood 暴露：①运行中的环境不按重命名后的别名显示（改名只刷了登记列表，运行实例行没跟上）；②/reset 语义存疑（是否清上下文、是否新建轨迹文件——文档与实现对不上）；③版本展示面漏改（README/其他版本面仍 1.6.5）；④漏洞研究（复现/分析）有效，但**漏洞挖掘**（fuzz 发现新 bug）效果不大——需分析全量轨迹找病灶。

- [ ] **#1 运行中环境别名显示**：改名后运行实例行/会话锚/终端标题全链显示别名
- [ ] **#2 /reset 语义核查与对齐**：查清实现（上下文/轨迹文件行为），文档与实现对齐，不符预期则修
- [ ] **#3 版本展示面归一**：README 及所有版本展示面同步 1.6.6（发版清单补「版本面核查」一项防再犯）
- [ ] **#4 漏洞挖掘有效性分析**：读全量研究轨迹（research_events + 会话轨迹 + fuzz 产出记录），定位「挖掘无效」的环节（fuzzer 没跑起来 / harness 质量 / 语料 / 中断过早 / 崩溃没接上 triage），出结论与修复项
- [ ] **#5 修复落地**：分析结论转修复任务，逐项过堂

> 边界：#4 先做分析后定修复——不预设改法；轨迹分析只读本机数据（~/.zhishi 研究档案/会话），不外发。
> 验收：#1~#3 有测试证据；#4 出书面结论（轨迹引用为证）；#5 修复项回归绿。
