...
- [ ] **M0 通道层补全**：能力探测 PowerShell 协议（`recipes.ts` TOOL_PROBE_COMMANDS/buildToolCheckScript 按 osFamily 分派 + `capability-derive.ts` 接线）；`resolveEnvOpenCommand` 消费 osFamily；extract basename 兼容 Windows 路径；vmrun `copyFileToGuest` 封装 + `environment/push` 端点；vm-guest-exec windows 分支补单测（现零覆盖）。**验收门**：手备 Win VM（OpenSSH Server + VMware Tools）登记后 up/exec/open/push/extract/探测全通
