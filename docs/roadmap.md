...
## 1.3.0 —— GUI 会话视图（MVP，进行中）

方向重大调整（用户拍板 2026-08-24）：**TUI 渲染层止损，转 GUI**。理由：自研终端渲染引擎的维护税已现（1.2.8/1.2.9 共 28 个渲染 bug）；「人机决策面」的形态（块/面板/折叠/选择）天生是 webview 的；sidecar + SSE + admin API 的契约本就是客户端无关的（M4a 红利）。顺序：**GUI 先做 MVP → 迭代 → 覆盖核心场景后退役 TUI**（不双份并行维护、不做迁移层）。

**版本路线（用户拍板）**：先原型后代码；**GUI 完成前不编译不发版**。共 5 版：

| 版本 | 内容 |
|---|---|
| 1.3.0（本版） | 交互原型（可点击 HTML，假数据走交互）+ 设计稿定稿 |
| 1.3.1 | GUI 骨架：Tauri 开窗 + webview 加载 + sidecar 联动 + SSE 会话流跑通（读/发/状态行） |
| 1.3.2 | 会话视图：块化流（输入=块首、结论聚合、细节折叠展开）、流式渲染、中断/纠偏、历史加载 |
| 1.3.3 | 决策面板（越界 + 分歧提请 + 专家依据）、历史面板、命令/引用面 |
| 1.3.4 | 打磨：侧栏、主题、体验细节；TUI 退役评估（覆盖核心场景后） |

- [ ] **1.3.0 原型**：可点击 HTML 原型（模拟数据，能实际点展开/折叠/发送），定交互后落设计稿。原型范围含**环境选择三态**（用户确认）：①首次启动无绑定环境 → 环境向导模态（选择已有/新建配方/手动 SSH，同 TUI gate 三入口，数据与能力复用 admin API）；②日常切换 → 顶栏切换器（1.3.1），1.3.3 升级为环境线=侧栏/tab（点线即切）；③环境管理（VM 启停/快照/rollback/接入/移除）收进环境行菜单。原型阶段顺带验证两个实现依赖：①SSE 直连 vs IPC 转发；②块数据语义（纯前端 vs 后端补字段）。
- [ ] **TUI 冻结**：不投功能/视觉，只修致命 bug。

> 明确不做（本版）：GUI 产品代码（等原型定稿）、TUI 块化与视觉投入（已止损）、决策面板（1.3.3 做，GUI 原生）、信息面板、侧栏（1.3.3 环境线形态）、主题（1.3.4）、编译发版（GUI 完成前）。
> 技术面待定稿：窗口/IPC/sidecar 联动（Rust 侧）、SSE 直连 vs IPC 转发、块数据语义（输入=块首/结论聚合/细节清单）、**多环境分层：A「一屏多线」=界面层多会话视图（1.3.3 随 GUI 迭代）vs B「多线真并行」=引擎层并发 turn（单独立版——现引擎单例全局一次只跑一个 turn，需改调度/SSE 分流/资源并发安全，原 2.0 前置项）**。
