...
- [ ] **1.3.0 原型**：可点击 HTML 原型（模拟数据，能实际点展开/折叠/发送），定交互后落设计稿。原型阶段顺带验证两个实现依赖：①SSE 直连 vs IPC 转发；②块数据语义（纯前端 vs 后端补字段）。
- [ ] **TUI 冻结**：不投功能/视觉，只修致命 bug。

> 明确不做（本版）：GUI 产品代码（等原型定稿）、TUI 块化与视觉投入（已止损）、决策面板（1.3.3 做，GUI 原生）、信息面板、侧栏（1.3.4）、主题（1.3.4）、编译发版（GUI 完成前）。
> 技术面待定稿：窗口/IPC/sidecar 联动（Rust 侧）、SSE 直连 vs IPC 转发、块数据语义（输入=块首/结论聚合/细节清单）。
