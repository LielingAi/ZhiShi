...
- [ ] **M1 配方 ps1 化 + `pwn-win` 配方**：frontmatter 认 `setup.ps1`；`provision.ts` 补齐链路 PowerShell 分支（`-EncodedCommand` 替代 base64|bash，sudo 预检换管理员组判定）；`bundled-environments/pwn-win/`（SKILL.md + setup.ps1，幂等可重放；包管理器 choco 兜底 + winget 优先——裸 VM 常缺 App Installer；必装段：Git/Python/LLVM(clang-cl)/VS Build Tools(VCTools)/cdb(SDK Debugging Tools)/Sysinternals/Ghidra headless/procdump；可选段：WinAFL+DynamoRIO、x64dbg；`_NT_SYMBOL_PATH`/WER LocalDumps/gflags PageHeap 封装；Defender 排除项引导）。**验收门**：实机重放 setup.ps1 幂等，探测全 OK
