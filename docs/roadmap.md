...
**缘起（2026-09-01 用户拍板）**：现状是容器为主——envUp 每次都跑 docker build（靠层缓存秒过）、down 即删容器；用户定调「正常的 docker 环境应该是镜像，镜像派生出容器」。1.5.9 的超时误报修复（BuildKit daemon 语义）已把「假重建耗时」修掉，本版把语义转正：镜像是一等公民，容器是镜像的派生实例。

- [ ] **up 改镜像优先**：ps 幂等检查（在跑则返回）→ `docker image inspect` 镜像在且配方内容哈希一致 → 直接 run（跳过 build）；镜像不在或哈希不一致 → build → run
- [ ] **配方内容哈希落盘**：Dockerfile + setup.sh + provision.sh + first-run.sh + 其余配方文件内容哈希，build 时打进镜像 label（`zhishi.recipe-hash`）；up 时 inspect 读 label 比对——配方更新（新版发行）自动识别重建，内容没变不重建
- [ ] **显式重建入口**：GUI 环境行/详情加「重新构建」（强制 build 不看哈希）——配方没变但镜像坏了的逃生门
- [ ] **down 语义不动**：容器仍临时（stop+rm）——镜像持久，容器是派生物（与本版主题一致）
- [ ] **回归 + 验证**：envUp 三分支单测（镜像在+哈希同跳过 build / 哈希不同重建 / 镜像不在构建）+ 幂等/超时恢复既有用例全绿；全量 + typecheck + eslint + 三构建；用户实跑（二次 up 秒开、配方改动后自动重建）

> 取舍记录在案：镜像持久占盘是代价（docker system prune 是用户的既有出口）；1.5.9 的 inspect 超时恢复逻辑保留为 build 分支的兜底。
> 边界（不做）：容器复用（stop/start 而不是 rm/run）——容器内状态可能带上一局研究残留，rm/run 派生新容器是干净语义；镜像仓库化（推送/共享）远超本版。
> 验收：二次 up 不触发 build（秒开）；配方内容变化自动重建；强制重建入口可用。
