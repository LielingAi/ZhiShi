...
- [ ] **M2 Windows adopt**：`vm-adopt.ts` Windows 路径——Add-WindowsCapability 启用 OpenSSH Server → 建 `researcher` 用户 → 公钥落位（`administrators_authorized_keys` 怪癖：管理员用户 key 落 ProgramData 不落用户目录，ICACLS 权限位）→ 跑 setup.ps1 → `shutdown /s` → `zhishi-clean` 快照。**验收门**：干净 Win VM 一条 `env adopt` 养成；之后 `env up` 回滚干净现场——与 Linux 体验同构
