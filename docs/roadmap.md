...
- [ ] **harness 确定性注入（唯一注入路径；图论口径）**：**注入 = 以当前焦点节点为锚的邻域子图投影**——节点 = 档案实体/专家条目/消息/工具，边 = 引用/驱动/证据/域先验/关键词相似（弱边即时计算，不建持久图）；每轮取焦点（pending H# / open Q# / 最近用户消息）的 k=1~2 邻域，边类型加权（证据边 > 派生边 > 域先验边 > 关键词相似边）+ 会话内去重（同条目只注一次）→ 注入 1-2 条（criteria 截断 + 总字符硬顶）；**零注入语义**（邻域为空静默）；**透明标注**（流内可见，注错人可纠正）；确定性（同输入同输出）；auto loop 同规则。
