...
- promote 常态化：真实研究结束后，人把验证过的经验晋升进库（评审工作流打磨；
  研究结案时 agent 主动提示「这条经验值得晋升吗？」）
- 引用追踪：research_log 可关联 expert entry id（「本次决策依据 E#12」）；
  报告（1.2.0 出口）标注引用的专家知识——知识的使用变得可审计
- 冲突记录：LLM 判断与专家知识冲突时记录冲突点（谁对谁错都是学习材料）
- 缺口识别教学打磨（1.2.1 实战遗留）：时机语义已定稿（先尽力，识别缺口才查；
  进展慢不是缺口）——本期把各域的缺口信号写成可对照的实例清单。
- 顺带修缺陷（1.2.1 实测发现）：system skills 在纯 sidecar 模式（无 Tauri
  宿主）不随包更新——CLI-only 用户永远拿不到 skill 更新。
