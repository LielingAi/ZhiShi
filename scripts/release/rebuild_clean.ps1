...
Write-Host "║  🔧 ZhiShi 彻底清理重建                              ║" -ForegroundColor Cyan
...
$required = @("http://ipc.localhost", "asset:", "connect-src", "https://download.zhishi.help")
...
if (-not $SkipUninstall) {
    ...
    $app = Get-WmiObject -Class Win32_Product | Where-Object { $_.Name -like "ZhiShi*" } | Select-Object -First 1
...
$webviewCache = "$env:LOCALAPPDATA\ZhiShi\EBWebView"
...
Get-Process | Where-Object { $_.ProcessName -eq "ZhiShi" } | ForEach-Object {
...
$cleanDirs = @(
    ...
    "src-tauri\target\x86_64-pc-windows-msvc\release\build\zhishi-*",
