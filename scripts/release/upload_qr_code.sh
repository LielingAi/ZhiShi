#!/bin/bash
# ./scripts/release/upload_qr_code.sh - Upload user community QR code to R2 storage
# Usage: ./scripts/release/upload_qr_code.sh
#
...
#
# Prerequisites:
# 1. .env 中配置了 R2 凭证（与 ./scripts/release/publish_release.sh 共用）：
#    - R2_ACCESS_KEY_ID
...
# Configuration - same bucket as ./scripts/release/publish_release.sh (download.zhishi.help domain)
R2_BUCKET="zhishi-releases"
TARGET_PATH="assets/feedback_qr_code.png"
PUBLIC_URL="https://download.zhishi.help/${TARGET_PATH}"

echo -e "${CYAN}╔════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║     ZhiShi QR Code Upload Tool               ║${NC}"
echo -e "${CYAN}╚════════════════════════════════════════════════╝${NC}"
echo ""

# ========================================
# Load environment variables (same as ./scripts/release/publish_release.sh)
# ========================================
...
# Configure rclone (same pattern as ./scripts/release/publish_release.sh)
...
# Upload using rclone (same as ./scripts/release/publish_release.sh)
