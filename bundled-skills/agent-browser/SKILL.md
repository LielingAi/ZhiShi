...
# ~/.zhishi/bin/agent-browser, satisfies `command -v`, but execs a deleted
...
agent-browser --version >/dev/null 2>&1 || npm_config_prefix="${ZHISHI_NPM_GLOBAL_PREFIX:-$HOME/.zhishi/npm-global}" npm install -g agent-browser@0.15.1
```

The install lands in `~/.zhishi/npm-global/bin/agent-browser`, which sits earlier in PATH than any legacy wrapper. ZhiShi exposes `ZHISHI_NPM_GLOBAL_PREFIX` for this command-local install instead of setting `npm_config_prefix` on the whole shell, so nvm-based user shells stay quiet. Subsequent `agent-browser …` calls find the new binary automatically.
...
**Requirements:** macOS with Xcode, Appium (`npm_config_prefix="${ZHISHI_NPM_GLOBAL_PREFIX:-$HOME/.zhishi/npm-global}" npm install -g appium && appium driver install xcuitest`)
