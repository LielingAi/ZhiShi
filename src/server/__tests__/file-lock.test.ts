...
beforeEach(() => {
  ...
  scratch = mkdtempSync(join(tmpdir(), 'zhishi-file-lock-'));
...
describe('withFileLock', () => {
  ...
  // macOS `ps`, Windows PowerShell since 1.6.3). Platforms without any — skip

  // the precise reuse assertion there.

  const startTimeSupported = process.platform === 'linux' || process.platform === 'darwin' || process.platform === 'win32';
