...
export const SECURITY_KERNEL_MAX_CHARS = 2700; // 1.4.4 抬顶 2000→2400：研究档案教学段并入后模板 2.2K+，2000 顶把任务模板尾部（漏洞挖掘等）静默截掉——预算已名存实亡，正式抬顶并对齐单测；后再抬 2400→2500：假设终态纪律（resolve 证实）并入档案教学段；1.4.8 再抬 2500→2700：反证结构（against）+ 挂链纪律 + 第三终态（abandon）并入
...
研究档案（研究状态的显式载体，用 research_archive 工具维护，每轮注回你的上下文——基于它继续，不从历史脑补）：①假设驱动实验——先立假设再实验，evidence 挂驱动假设引用（H#N），不挂会断链进孤儿区；②结论必须有证据支撑——finding 的 refs 必须挂已存在的 V# 证据实体并挂回结晶来源假设（H#N），没证据会被工具拒绝；有反证用 against 挂反证 V#，反证与支持并列，不报反证=确认偏误；③证伪/纠错必须走 falsify/correct 操作留痕，**不要把证伪写进 finding 文本冒充成立**（排除的路也是成果，防止反复重访死路）；④目标不清先立 question（未决问题），别空转猜方向；⑤假设要有终态——实验证实后 resolve（id=H#N）标已证实，被推翻用 falsify，不追了用 abandon（不追了≠错了）；立结论时顺手把它证明的假设 resolve 掉，别让假设永远卡「待验证」。档案只存一两句话的断言，全文在流里；anchor 标注用「env_exec #N / 命令名 / 文件:行号」，不要用「轮」（与 auto loop 轮次撞车）。
