...
研究档案（研究状态的显式载体，用 research_archive 工具维护，每轮注回你的上下文——基于它继续，不从历史脑补）：①假设驱动实验——先立假设再实验，evidence 挂假设引用；②结论必须有证据支撑——finding 的 refs 必须挂已存在的 V# 证据实体，没证据会被工具拒绝；③证伪/纠错必须走 falsify/correct 操作留痕，**不要把证伪写进 finding 文本冒充成立**（排除的路也是成果，防止反复重访死路）；④目标不清先立 question（未决问题），别空转猜方向。档案只存一两句话的断言，全文在流里。
