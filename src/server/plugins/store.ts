...
import { listPluginLicenses } from '../memory/store';
 ...
 * Reserved plugin names — they would collide with `~/.zhishi/plugins/`'s
...
/** Resolved plugins root: ~/.zhishi/plugins/ */

export function getPluginsRoot(): string {

  return join(getZhiShiDataDir(), 'plugins');

}



/** ~/.zhishi/plugins/data/ (parent for ${CLAUDE_PLUGIN_DATA}) */
...
export async function installPlugin(
  ...
  // ~/.zhishi/plugins/<name> and ran `cc-plugin install file://…/<name>`.
...
export function listInstalledPlugins(): PluginListItem[] {
  ...
  // .zsp 加密插件的激活信息（memory.db plugin_licenses 联查，

  // specs/tech_docs/encrypted_plugins_t1.md）。只在 memory.db 已存在时才打开

  // ——列表是读路径，不能为了联查把 db 文件创建出来（也避免 Windows 上 sqlite

  // 句柄锁文件带来的清理问题）。读取失败降级为无 license 对象，不影响主流程。

  let licenseByPluginId: Map<string, { publisher: string; activatedAt: number }> | null = null;

  if (existsSync(join(getZhiShiDataDir(), 'memory.db'))) {

    try {

      licenseByPluginId = new Map(

        listPluginLicenses().map(r => [r.pluginId, { publisher: r.publisher, activatedAt: r.activatedAt }]),

      );

    } catch (err) {

      console.warn('[plugins] listInstalledPlugins: plugin_licenses 读取失败（忽略）:', err);
  ...
  return entries.map(entry => {
    ...
    return {
      ...
      // 加密插件：用 plugin_licenses 的激活信息覆盖 plugin.json 的 SPDX 串

      // （PluginListItem.license: string | PluginLicenseInfo，UI 按对象形态识别 🔒）

      license: licenseByPluginId?.get(entry.id) ?? entry.license,
...
export function getEnabledPluginSdkConfigs(
  ...
  for (const p of entries) {
    ...
    // ~/.zhishi/plugins/foo → /tmp/evil, we won't hand /tmp/evil to SDK.
...
export function getDefaultEnabledPluginIdsForWorkspace(workspacePath: string): string[] {
  ...
  try {
    ...
    const projectsPath = resolve(getZhiShiDataDir(), 'projects.json');
