 ...
 *   C:\Users\xxx\AppData\Local\ZhiShi\
...
// v0.2.0: Bun path-discovery helpers removed. ZhiShi no longer bundles Bun;
...
/**
 * Get the absolute path to the bundled Terminator MCP agent binary
 * (mediar-ai/terminator -- UIA desktop automation engine, AppCraft PRD 0.2.36).
 *
 * Windows-only (upstream is Windows-only). Layout mirrors cuse:
 * - Windows (prod): <install-dir>/terminator-mcp-agent.exe (flat, next to cuse.exe)
 * - Dev:            <project>/src-tauri/binaries/terminator-mcp-agent-<triple>.exe
 *
 * Returns null when the binary is missing (non-Windows platform, or a dev build
 * without the binary yet) -- the caller skips the MCP with a warning.
 */
export function getBundledTerminatorPath(): string | null {
  // Hard platform gate: terminator is Windows-only.
  ...
  // Production: flat layout next to cuse.exe / server-dist.js
  const prodBin = resolve(scriptDir, 'terminator-mcp-agent.exe');
  if (existsSync(prodBin)) return prodBin;

  // Development: walk up from scriptDir to find src-tauri/binaries/terminator-mcp-agent-<triple>.
  let dir = scriptDir;
  for (let i = 0; i < 6; i++) {
    const devBin = resolve(dir, 'src-tauri', 'binaries', 'terminator-mcp-agent-x86_64-pc-windows-msvc.exe');
    if (existsSync(devBin)) return devBin;
    dir = dirname(dir);
  }
...
/**
 * Absolute path to the bundled better-sqlite3 entry (`lib/index.js`) — same
 * layout contract as getBundledSharpEntryPoint:
 *   production: <Resources>/sqlite-runtime/node_modules/better-sqlite3/lib/index.js
 *   dev:        node_modules/better-sqlite3/lib/index.js (or src-tauri/resources/sqlite-runtime)
 * better-sqlite3 ships a per-ABI native addon (build/Release/better_sqlite3.node)
 * that esbuild cannot bundle — it is rebuilt against the bundled Node's ABI at
 * packaging time (scripts/dev/rebuild-sqlite.ps1).
 */
export function getBundledSqliteEntryPoint(): string | null {

  const relBundled = join('sqlite-runtime', 'node_modules', 'better-sqlite3', 'lib', 'index.js');
  ...
  const relDev = join('node_modules', 'better-sqlite3', 'lib', 'index.js');
