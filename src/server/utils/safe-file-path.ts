 ...
 *   - `~/.zhishi/tmp` (scratch space, where AI writes intermediate files)
...
export interface SafeFilePathOptions {
  ...
  /**
   * Additional allowed roots. Defaults to `~/.zhishi/tmp` and the platform
...
export function assertSafeFilePath(filePath: string, options: SafeFilePathOptions): string {
  ...
  // resolves to. Without this, `~/.zhishi/tmp/link → /etc/shadow` slips
  ...
  if (!allowed) {
    throw new Error(
      `path "${filePath}" is outside the allowed roots. Only files under the current workspace, ~/.zhishi/tmp, or the system temp directory can be read for this operation.`
    );
  }
...
function defaultExtraRoots(_home: string | null): string[] {
  const roots = [tmpdir()];
  roots.push(resolve(getZhiShiDataDir(), 'tmp'));
  return roots;
}
