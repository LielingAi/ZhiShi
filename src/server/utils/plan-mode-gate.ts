 ...
 * WHY THIS EXISTS
 * ---------------
 * ZhiShi launches the Claude Agent SDK subprocess with
 ...
 * `canUseTool` callback (where ZhiShi enforces the plan-mode read-only rules)
 ...
 * `mcp__*` tools, the `zhishi` CLI, …). Returns `false` (allow normal flow)
