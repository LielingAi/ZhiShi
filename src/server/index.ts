...
import { ZspError } from '../shared/zsp-crypto';



// M4c: schedulePluginDeferredRestart 随 SDK 会话机删除——

// pi 引擎不消费插件运行时注入,插件变更无需会话重启调度。
...
const SYSTEM_SKILLS: readonly string[] = [
  ...
  // v26: plugin-assistant — 插件制作向导（编写/打包/加密/发布），

  // 与 zhishi plugin init/pack/keygen/verify 本地子命令同步演进。
 ...
 * Ensure the ~/.zhishi/plugins/ directory tree exists for Claude Plugin
 ...
 *   ~/.zhishi/plugins/                  plugin install root

 *   ~/.zhishi/plugins/data/             ${CLAUDE_PLUGIN_DATA} parent
...
function ensurePluginsDirs(): void {
  ...
  try {
    ...
    const root = join(getZhiShiDataDir(), 'plugins');
...
async function routeAdminApi(pathname: string, payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  if (route === 'cc-plugin/install-zsp') return await api.handleCcPluginInstallZsp(payload as Parameters<typeof api.handleCcPluginInstallZsp>[0]);
...
async function main() {
  ...
  async function handleRequest(request: Request): Promise<Response> {

    {
      ...
      // POST /api/cc-plugin/install-zsp - 加密插件（.zsp）验签解密安装 +

      // 许可激活（specs/tech_docs/encrypted_plugins_t1.md）。ZspError 的

      // code 透传给 UI（按 code 分行中文文案），PluginStoreError 沿用明文

      // install 的状态码语义（409 冲突等）。

      if (pathname === '/api/cc-plugin/install-zsp' && request.method === 'POST') {

        try {

          const body = (await request.json()) as { filePath?: string; license?: string };

          if (!body.filePath || typeof body.filePath !== 'string') {

            return jsonResponse({ success: false, error: 'filePath 参数必填' }, 400);

          }

          if (!body.license || typeof body.license !== 'string') {

            return jsonResponse({ success: false, error: 'license 参数必填' }, 400);

          }

          const plugin = await installZspPlugin(body.filePath, body.license);

          broadcast('plugins:changed', { reason: 'install' });

          // M4c: 插件安装不再需要 SDK 会话重启调度。

          return jsonResponse({ success: true, plugin });

        } catch (error) {

          if (error instanceof ZspError) {

            return jsonResponse({ success: false, error: error.message, code: error.code }, 422);
          ...
          if (status >= 500) {
            ...
            console.error('[api/cc-plugin/install-zsp] Error:', error);
      ...
      if (pathname === '/api/cc-plugin/uninstall' && request.method === 'POST') {
        ...
        try {
          ...
          // .zsp 加密插件的许可行一并清掉（重装需重新激活——spec §4.3）。

          // 幂等删除，失败仅告警，不阻断卸载主流程。

          try {

            const { deletePluginLicense } = await import('./memory/store');

            deletePluginLicense(body.id);

          } catch (err) {

            console.warn('[api/cc-plugin/uninstall] deletePluginLicense failed (non-fatal):', err);
      ...
      if (pathname === '/api/cc-plugin/session-enable' && request.method === 'POST') {
        ...
        try {
          ...
          // M4c: 插件的会话注入随 SDK 删除——启用状态只落配置面(此端点不再写会话)。

          console.log('[api/session/enabled-plugins] M4c: 会话注入已删除,配置面由插件管理端点维护');
