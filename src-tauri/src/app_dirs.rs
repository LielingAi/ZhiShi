...
//!
//! All code that needs `~/.zhishi/` SHOULD use [`zhishi_data_dir()`] instead of
...
//! ## PID Lock File
//!
//! `~/.zhishi/app.lock` contains the PID of the running ZhiShi instance.
//! - Written by [`acquire_lock()`] during app startup (after single-instance check).
//! - Read by build scripts (`./scripts/dev/build_dev.sh`, `./scripts/dev/start_dev.sh`) to precisely kill the
...
pub fn record_clean_exit(is_restart: bool) {
    let Some(dir) = zhishi_data_dir() else { return };
...
/// Outcome of [`acquire_lock`] — encodes whether a prior ZhiShi instance
...
pub enum LockAcquireResult {
    ...
    FreshLaunch,
    /// Lock file existed and pointed at a live ZhiShi process — we killed
...
impl LockAcquireResult {
    /// `true` if a prior ZhiShi instance may have left orphaned child
...
/// Return the ZhiShi data directory (`~/.zhishi/` by default).
///
/// Future: debug builds may return `~/.zhishi-dev/` to enable simultaneous
/// dev/prod operation with fully isolated state (config, bots, sidecars, ports).
/// For now, both profiles share the same directory.
/// Check if the app is running in portable mode by looking for a `.data_mode`
/// marker file (written by the installer) or an existing `.zhishi/` directory
/// next to the executable (ZIP extract-and-run).
fn resolve_portable_data_dir() -> Option<PathBuf> {
    let exe_path = std::env::current_exe().ok()?;
    let exe_dir = exe_path.parent()?;

    // 1. Explicit marker file written by the NSIS installer in USB mode.
    let mode_file = exe_dir.join(".data_mode");
    if mode_file.exists() {
        if let Ok(mode) = std::fs::read_to_string(&mode_file) {
            if mode.trim() == "portable" {
                return Some(exe_dir.join(".zhishi"));
            }
        }
    }

    // 2. A standard install (no `.data_mode`) MUST keep reading the fixed home
    //    directory. The implicit exe-side probe below must NOT win while a home
    //    `.zhishi` exists: an empty `.zhishi` can appear next to the exe without
    //    user intent (e.g. an updater log dir), and hijacking the data dir then
    //    makes the user's real history "disappear". Home data is authoritative
    //    whenever it is present.
    if let Some(home) = dirs::home_dir() {
        let home_dir = home.join(".zhishi");
        if home_dir.is_dir() {
            return Some(home_dir);
        }
    }

    // 3. Implicit: `.zhishi/` directory already exists next to the executable.
    //    Only consulted when there is no home `.zhishi` — covers the "extract
    //    ZIP and run" portable distribution scenario where no installer (and
    //    therefore no `.data_mode` file) is involved.
    let portable_dir = exe_dir.join(".zhishi");
...
/// Return the ZhiShi data directory.
///
/// Resolution order (highest → lowest priority):
/// 1. `ZHISHI_DATA_DIR` environment variable — used by sidecars and CLI
///    subprocesses so they resolve the same directory as the main Rust process.
/// 2. Portable mode: `.data_mode` marker file or `.zhishi/` directory next
///    to the executable (USB / ZIP distributions).
/// 3. Default: `~/.zhishi` (standard local install).
pub fn zhishi_data_dir() -> Option<PathBuf> {
    // 1. Environment variable override (highest priority)
    if let Ok(env_dir) = std::env::var("ZHISHI_DATA_DIR") {
        if !env_dir.is_empty() {
    ...
    dirs::home_dir().map(|h| h.join(".zhishi"))
}

/// Path to the PID lock file.
fn lock_file_path() -> Option<PathBuf> {
    zhishi_data_dir().map(|d| d.join("app.lock"))
}

/// Write the current process PID to `~/.zhishi/app.lock` and report
/// whether a prior instance's state was encountered.
///
/// If an existing lock file contains a PID of a still-running ZhiShi process,
        ...
        Ok(content) => {
            match content.trim().parse::<u32>() {
                Ok(old_pid) => {
                    ...
                    } else if is_zhishi_process(old_pid) {
                        ulog_warn!(
                            "[app-lock] Killing stale ZhiShi instance (PID {}) before acquiring lock",
                            old_pid
                        );
                        kill_pid(old_pid);
...
/// Check if a PID belongs to a running ZhiShi process (not just any process).
...
fn is_zhishi_process(pid: u32) -> bool {
    ...
    crate::process_cleanup::is_zhishi_pid(pid)
}
...
fn kill_pid(pid: u32) {
    ...
    // Caller has already verified this PID is a ZhiShi process.
    unsafe {
