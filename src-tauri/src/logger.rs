...
// `local_http.rs` to propagate `X-ZhiShi-Request-Id /
// X-ZhiShi-Session-Id / X-ZhiShi-Tab-Id` from inbound headers.
...
/// Get logs directory path (~/.zhishi/logs/)
fn get_logs_dir() -> PathBuf {
    static LOGS_DIR: OnceLock<PathBuf> = OnceLock::new();
    LOGS_DIR
        .get_or_init(|| {
            let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
            home.join(".zhishi").join("logs")
        })
        .clone()
}
