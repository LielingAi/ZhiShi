...
//! stores it raw under `~/.zhishi/cache/`. The Node sidecar reads it as the
...
fn cache_dir() -> Result<PathBuf, String> {
    ...
    Ok(home.join(".zhishi").join("cache"))
}
...
/// Read the toggle from `~/.zhishi/config.json`. Default ON: a missing file,
/// missing key, or parse error all mean "enabled" — the feature opts users IN.
fn is_enabled() -> bool {
    let Some(home) = dirs::home_dir() else {
        return true;
    };
    let path = home.join(".zhishi").join("config.json");
...
fn build_client() -> Result<reqwest::Client, String> {
    ...
    let builder = reqwest::Client::builder()
        ...
        .user_agent(concat!("ZhiShi/", env!("CARGO_PKG_VERSION")));
