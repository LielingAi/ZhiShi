...
4. **崩溃收集**：崩溃在 `fuzz/corpus-out/default/crashes/`（`README.txt` 之外的 `id:*` 文件）。**去重**（信号 + 顶帧站点 hash）后拷入工作区 `fuzz/crashes/unique/`，每个样本附一份 `.meta`（信号、afl id、复现命令）。崩溃的逐类根因研判是 crash-triager 的活，你只做初判（见下）。crash-triager 深挖产出的验证变体若落在 `fuzz/corpus-in/`，下一轮启动时带上（这是变体回灌的闭环，别清掉）。
